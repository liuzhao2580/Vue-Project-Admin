...
import { add, getModel } from "@/api/workflow/model";
...
export default {
  ...
  methods: {
    ...
    highlightedCode(code, key) {
      ...
      const result = hljs.highlight(key, code || "", true);
      return result.value || "&nbsp;";
    },
    /* 获取流程xml */
    getModel() {
      ...
      if (id) {
        getModel(id).then(res => {
          if (res.code === 200) {
            let xmlStr = res.data.modelValue;
    ...
    // 提交保存
    submit() {
      let rid = this.$route.query?.id; // 路由传来的流程id
      let processKey = this.$store.state.app.processKey; // 重新编辑的流程key
      if (rid) {
        this.bpmnModeler.saveXML({ format: true }).then(({ xml }) => {
          let elementRegistry = this.bpmnModeler.get("elementRegistry");
          let businessObject;
          if (processKey) {
            let data = elementRegistry.get(processKey);
            if (data) {
          ...
          add({
            ...
            name: processKey
              ? businessObject
                ...
                : ""
              : "",
            key: processKey ? (businessObject ? businessObject?.id : "") : ""
          }).then(res => {
            if (res.code === 200) {
    ...
    initBpmnModeler() {
      ...
      this.bpmnModeler = new BpmnModeler({
        container: this.$refs["bpmn-canvas"], // 渲染容器
        keyboard: this.keyboard ? { bindTo: document } : null,
        additionalModules: this.additionalModules, // 自定义且与默认扩展包一起使用的模块列表
        moddleExtensions: this.moddleExtensions, // 需要用的扩展包
        ...this.options
      });
