...
<template>
  <div style="margin-top: 16px">
    <el-form-item label="处理用户">
      ...
      <el-button @click="dialogVisible = !dialogVisible">选择处理用户</el-button>
    </el-form-item>
    ...
    <el-dialog title="选择处理用户" :visible.sync="dialogVisible" width="500px" append-to-body>
...
<!--            <el-radio label="$var">变量</el-radio>-->
        ...
        <el-form-item label="审批人员" prop="personName">
          <el-select v-model="form.personName" v-if="!showTree" clearable>
            <template v-if="!isVar">
              <el-option v-for="(option, index) in options" :key="index" :label="option.userName || option.roleName" :value="option.userName || option.roleKey" />
            </template>
            <template v-else>
                <el-option
          ...
          <treeselect v-else v-model="form.personName" :options="options" clearable placeholder="请选择部门菜单" />
        </el-form-item>
      </el-form>
  ...
  methods: {
    ...
    submitForm() {
      if(this.form.personType === "$dept") {
        this.userTaskForm.assignee = this.form.personType + ':' + this.form.personName
      }else {
