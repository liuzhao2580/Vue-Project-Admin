<template>
  <div class="AmapCom-box">
    ...
    <el-card class="AMap-toolBar" v-show="positionStatus">
      <p class="status">{{ getPositionInfo.status }}</p>
      <el-divider></el-divider>
      <p class="position">定位坐标: {{ getPositionInfo.position }}</p>
      <p class="formattedAddress">
        定位地址: {{ getPositionInfo.formattedAddress }}
      </p>
      <p class="adcode-p">当前邮编: {{ getPositionInfo.adcode }}</p>
      <p class="location_type">定位类别: {{ getPositionInfo.location_type }}</p>
    </el-card>
    <!-- 定位按钮 -->
