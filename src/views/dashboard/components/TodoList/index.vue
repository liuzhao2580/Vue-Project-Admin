<template>
    <div class="todo-list">
        ...
        <main class="main-box">
            <!-- 输入框 -->
            <div class="input-box">
                <el-input v-model="insertInput" placeholder="请输入内容" clearable @keyup.enter.native="handleInsertInput"></el-input>
            </div>
            <!-- todo 展示列表 -->
            <div class="list">
                <div v-for="(item, index) in checkedList" :key="index" @click="handleItemClick(index)" class="list-item" @mouseenter="handleMouseEnter(index)" @mouseleave="handleMouseLeave">
                    <span :class="[item.flag ? 'icon-xuanzhong' :'icon-xuanze', 'iconfont list-icon']"></span>
                    <span class="list-title lineEllipsisOne">{{item.title}}</span>
                    <span class="iconfont icon-guanbi close-btn" v-show="colse_isActive == index" @click.stop="removeList(index)"></span>
                </div>
            </div>
        </main>
