...
const creatRouter = (route: RouteRecordRaw, roleId: UserRolesEnum) => {
  ...
  // 处理子路由的情况
  function handleChildRoute(handleRoute: RouteRecordRaw) {
    // 2. 再判断 该页面是否存在子路由
    // 2.1 该用户有子路由，使用递归判断子路由是否可以访问
    if (handleRoute.children) {
      const getChildrenRouter = creatRouter(handleRoute, roleId)
      routesArr.push({
        ...handleRoute,
    ...
    // 2.2 说明该页面没有子路由
    else routesArr.push(handleRoute)
  }
